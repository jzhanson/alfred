...
parser.add_argument('--custom-resnet-layers', type=int, nargs='+', default=[2, 2, 2, 2], help='4 ints that determine how many residual blocks to use in each layer (default: 2 2 2 2)')
parser.add_argument('--custom-resnet-inplanes', type=int, default=64, help='number of input planes before residual layers - probably the same as the first custom-resnet-planes? (default: 64)')
parser.add_argument('--custom-resnet-planes', type=int, nargs='+', default=[64, 128, 256, 512], help='4 ints that determine how many filters/planes to use in each layer (default: 64, 128, 256, 512)')
...
parser.add_argument('--policy-loss-coefficient', type=float, default=1.0, help='coefficient for policy loss (default: 1.0)')
...
parser.set_defaults(navigation_superpixels=False)
parser.add_argument('--action-mask-score-combination', type=str, default='add', help='whether to \'add\' or \'multiply\' raw action and mask scores with --outer-product-sampling together with --navigation-superpixels (default: \'add\')')
...
parser.set_defaults(loading_interaction=False)
parser.add_argument('--dataset-type', type=str, default='imagenet', help='type of dataset to use  for training visual probe (\'imagenet\' or \'interaction_scene\' or \'interaction_object\')')
parser.add_argument('--interaction-scene-target-type', type=str, default='in_frame', help='type of target for interaction datasets (\'visible\' as defined by the environment and VISIBILITY_DISTANCE or \'in_frame\')')
...
# Creating datasets for supervised training, visual probes
...
parser.set_defaults(create_object_dataset=True)
...
parser.add_argument('--include-train-scenes', dest='include_train_scenes', action='store_true', help='include train scenes in building dataset')
parser.add_argument('--exclude-train-scenes', dest='include_train_scenes', action='store_false', help='do not include train scenes in building dataset')
parser.set_defaults(include_train_scenes=True)
parser.add_argument('--include-valid-scenes', dest='include_valid_scenes', action='store_true', help='include valid scenes in building dataset')
parser.add_argument('--exclude-valid-scenes', dest='include_valid_scenes', action='store_false', help='do not include valid scenes in building dataset')
parser.set_defaults(include_valid_scenes=False)
parser.add_argument('--include-test-scenes', dest='include_test_scenes', action='store_true', help='include test scenes in building dataset')
parser.add_argument('--exclude-test-scenes', dest='include_test_scenes', action='store_false', help='do not include test scenes in building dataset')
parser.set_defaults(include_test_scenes=False)
parser.add_argument('--cluttered-scene-threshold', type=int, default=7, help='number of objects beyond which a scene will be considered cluttered for rejection sampling probe datasets')
parser.add_argument('--cluttered-crop-threshold', type=int, default=3, help='number of objects beyond which an object crop will be considered cluttered for rejection sampling probe datasets')
parser.add_argument('--rejection-n', type=int, default=5, help='number of "exact same" trajectories and/or objects to keep before rejecting in rejection sampling probe datasets')
parser.add_argument('--balanced-object-dataset', dest='balanced_object_dataset', action='store_true', help='balance object dataset by only including object instances with new properties')
parser.add_argument('--all-objects-dataset', dest='balanced_object_dataset', action='store_false', help='include all objects from a trajectory in object dataset if one object has unseen properties')
parser.set_defaults(balanced_object_dataset=False)
